...
SB_training_data <- left_join(training_data_values, training_data_attributes,
                              # ID column on both variables
                              by = c("ID" = "id")) %>% 
                              # Converting landcover type to factor
...
dt_plot <- tm_shape(SB_classification_dt) +
  tm_raster(
    col.scale = tm_scale(
      values = c("#8DB580", "#F2DDA4", "hotpink", "#6A8EAE"),
...
# perform k means with. k = 4 (groups)
...
# Assign all values in that raster as NA
values(SB_classification_kmeans) <- NA 

# Repopulate it -  Take the x,y coordinate from landsat_df and convert them to cell numbers for the raster
...
# Cluster 3 = green vegetation - High NIR, low red
# cluster 4 = blue -  water is higher than NIR
# cluster 1 = soil/ dead grass - overall high reflectance, high numbers
# cluster 2 = urban - 
...
kmeans_plot <- tm_shape(SB_classification_kmeans) +
  tm_raster(
    col.scale = tm_scale(
      values = c("#F2DDA4","hotpink",  "#8DB580","#6A8EAE"),
