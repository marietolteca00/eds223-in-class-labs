...
View(zion_elevation)
# the 'x1' means it is only 1 layer
```
...
# Find out the number of layers in our raster object/ data
nlyr(landsat)

# Find out the names of layers in rasters
names(landsat)
```
...
```{r}
# Only looking at raster landsat_3
landsat3 <- subset(landsat, 3)

# Only looking at raster landsat_4
...
map1 <- tm_shape(austria) +
  tm_raster(col.legend = tm_legend("Elevation (m)"), col.scale = tm_scale_continuous(values = c(0,500,1000,1500, 2000,3000))) +
...
## **Where did we get the cordinates from** - From `erra::ext(zion_elevation)`
```{r}
...
```{r}
# Make sure extents are not the same since one is clipped!
if(ext(zion_elevation) == ext(zion_elevation_clip)){
print("extents match")
} else{
print("extents do not match") # This is what we want
}
...
# Both are correct approaches, results are the same!
...
- Can combine raster and vector together (1 lab)
...
- *Focal* Aggregate neighboring cells
- *Zonal* Aggregating based of 
*Calculating NDVI, Simple arithmatic*
```{r}
# Running zion_elevation in console

zion_elevation + zion_elevation # doubling each cell value, look at max value

zion_elevation^2 # look at max value
...
# Zion_Elevation is the PARAMETER
...
```{r}
# Define NDVI
ndvi_fun <- function(nir,red){ # near infrared layer
  (nir-red) / (nir+red)
}

# Apply NDVI Function to Landsat bands 3 & 4
ndvi_rast <- lapp(landsat[[c(4 , 3)]], fun = ndvi_fun)
# Need to view Metadata to know which data stands for
...
elevation_focal <- focal(zion_elevation,
                          # 1st Column will be of 1's 9x9 box
...
zonal(zion_elevation, reclassified, fun = "mean") %>% 
    kable(colnames = c("Elevation zone", "Mean Elevation (m)")) %>% 
...
# Getting an error - thats correct
```
...
map2 <- tm_shape(zion_elevation_coarse) +
  tm_raster(col.legend = tm_legend(title= "Elevation (m)"))+
  tm_title(text = "Aggregated") # Decreases Resolution
...
```{r}
# Disaggregating the aggregated raster
# 
