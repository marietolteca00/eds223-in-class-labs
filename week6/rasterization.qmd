...
boundary <- read_sf(system.file("vector/zion.gpkg", package = "spDataLarge")) # vector, GPKG package: SF object, spatial column, 'geom'
...
# Warning Message: check and Transform CRS
if(crs(elevation) == crs(boundary)){
  print("Coordinate reference system match :)")
} else{
  warning("Update coordinate reference to match")
...
```{r}
# Finding out if raster is a line or polygon
...
- its applicable with stars packages because it is not a part of the terra package
- Boundary was read in as `read_sf`, vector, use sf features
...
park_border <- boundary %>% 
  # period(.) is a place holder for boundary, no need to add boundary since it is being piped
...
```{r}
#| message: false
map1 <- tm_shape(park_polygon) + # polygon filled in
  tm_polygons() +
  tm_title(text = "polygon")

map2 <- tm_shape(park_border) + # not filled in 
  tm_lines() +
  tm_title(text = "line")
...
```{r}
polygon_raster <- terra::rasterize(park_polygon,
                                   # will match elevation, resolution, and extent of elevation
                                   elevation)
...
template_raster_high <- rast(ext(park_polygon), resolution = 0.01, crs = st_crs(park_polygon))
...
```{r}
# define point data- this comes from the spData package
...
template_raster <- rast(ext(cycle_hire_osm_projected),
                        # pixels, 1000 by 1000
                        resolution = 1000,
                        # crs of the code line above - matching crs
...
```{r}
# Rasterize resolution to match the defined raster template
...
```{r}
# allows for various summarizing functions.
ch_raster2<- rasterize(cycle_hire_osm_projected, template_raster, fun = 'length') #return a count of the number of points in each cell
```
...
- Near infrared (red channel), red (green channel), green (blue channel): **MAP2**
...
- Shortwave infrared (red channel), near infrared (green channel), green (blue channel):**MAP3**
...
map3 <- tm_shape(L7) +
  ...
  tm_title(text = "Shortwave IR, Near IR, green")
...
Normalized Difference Moisture Index: Vegetation index used in RS to measure and monitor the water content in vegetation

NDMI = (NIR-SWIR) / (NIR+SWIR)

- Measure moisture within plant levels:
- higher values = more water content in vegetation

(Use color = brewer.blues)

*Useful for drought monitoring and fire risk*
```{r}
# define NDMI as the normalized difference between green and nir bands
ndmi_fun <- function(nir, swir){
  (nir - swir) / (nir + swir)
}

# apply NDWI function to Landsat bands 4 & 5
...
tmap_arrange(map_ndwi, map_ndmi)
```
