...
```{r}
eaton_perimeter <- st_read(here::here( "week9/data/Eaton 3/Eaton_Perimeter_20250121"))%>% st_transform(crs = crs(dsm_post))
```
...
if(crs(dtm_pre) == crs(dsm_pre) &
  ...
  print('All coordinate reference system match!')
} else{
...
- ground altering - fire assessment- buildings burnt

```{r}
# First look at ground surface change
...
# Calculate Quantiles to determine breaks
quantiles <- global(diff_dsm_crop, fun = quantile,
                    probs = c(.005, .995), na.rm=TRUE)

# Using quantiles use to make upper and lower bound
# Remove values outside of upper and lower quantile
diff_dsm_scale <- clamp(diff_dsm_crop,
                        lower = quantiles[1,1], # row 1 column 1
                        upper = quantiles[1,2]) # row 2 column 2
...
# MAP - DSM Difference map
tm_shape(diff_dsm_scale)+
  # Because its a tiff
  tm_raster(
...
```{r}
# Select single block of homes in Altadena- black box
...
overlay <- height_shade(
  ...
  diff_mat,
  texture = colorRampPalette(c("#8B0000", "#FF6347", "#FFFFFF", "#87CEEB", "#00008B"))(256),#pick 256 colors
  # range 256 colors between this range
  range = c(-20,20)
)
...
elev_mat %>% 
  ...
  # Add realistic shadows at 50% intensity to enchance the 3D effect
...
```{r}
ggplot(data = damage_plot, aes(x = age_category, y = elevation_loss, fill = age_category))+
