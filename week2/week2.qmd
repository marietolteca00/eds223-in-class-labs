...
```{r}
# Create a data frame of attributes about London
...
```{r}
# combine geometry and dataframe
...
```{r}
world <- spData::world
# checking what the data class is
class(world)
...
```{r}
# Stats - Boxplot numbers, not plot
...
ncol(world_df) # Total columns - 11
```
...
ncol(world_df) # checking to see if one less column - YES
```
...
warning("coordinate reference system does not match!")
dataset2 <- st_transform(dataset1, crs = st_crs(dataset))
}

```
:::

\*\* We can also check if our crs is projected or geographic! Only two options- projected or geographic\*\*
...
arrange(desc(n_countries)) %>% #W/o adding 'desc' it does ascending!
st_drop_geometry()
...
```{r}
nrow(coffee_data) # Has 47 rows
nrow(world) # # Has 177 rows
```
...
# adding the data from coffee_data into world_coffee
names(world_coffee)

# checking crs for world coffee - 
st_crs(world_coffee)
```
...
warning("inner joing does not match original data. potential data loss during join")
}
