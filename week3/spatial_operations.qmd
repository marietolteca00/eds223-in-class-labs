...
# Filter to the region of Canterbury ( One Oberservation )
canterbury <- nz %>% 
...
```{r}
tm_shape(nz) +
  # Only looking at polygons
  tm_polygons() +
  
  # Calling filtered observation
  tm_shape(canterbury) +
  
  # Color of Canterbury Polygon
  tm_polygons(fill = "yellow") +
  
  # Calling nz_heights by
  tm_shape(nz_height) +
...
c_height1 <- nz_height[canterbury, ] # Rows of Canterbury, 
```
...
# No comma after y = , BC we only want the geometry of the rows NOT THE COLUMNS
...
# Spared Binary Predicate List
nz_height_sgbp <- st_intersects( x = nz_height, y = canterbury)
nz_height_sgbp # Empty = False, 1 = True

# Convert to a Logical Vector (T / F)
nz_height_logical <- lengths(nz_height_sgbp) > 0
nz_height_logical

# Filter based on the Logical Vector
c_height3 <- nz_height[nz_height_logical, ] # Same # of observations as c_height_1 and 2
```
...
map2 <- tm_shape(nz)+
  tm_polygons()+
  tm_shape(canterbury) +
  tm_fill(fill = "skyblue") +
  tm_shape(c_height2) +
  tm_dots(fill = "red") + 
  tm_title(text = "Used st_filter()")

map3 <- tm_shape(nz)+
  tm_polygons()+
  tm_shape(canterbury) +
  tm_fill(fill = "lightslateblue") +
  tm_shape(c_height3) +
  tm_dots(fill = "red") + 
  tm_title(text = "Used st_intersects()")
...
#                                            # Setting Distance of 1000 km unit
#                                            dists = unit::set_units(1000, "km"),
#                                            # Will Return as a logical Vector
#                                            spare = FALSE)
# # Filter based on logical vector
# c_height_4 <- nz_height[nz_height_logical, ]
 

# find heights within 1000 km of Canterbury
nz_height_logical <- st_is_within_distance(nz_height, canterbury,
                      dis = units::set_units(1000, "km"), # set distance
...
```{r}
# Check whether or not point overlap

if(any(st_intersects(cycle_hire, cycle_hire_osm, 
                     # Spare = false to get TRUE and FALSE
                     sparse = FALSE)) == TRUE){
  print("Points Overlap!")
} else {
  warning("Points DO NOT Overlap :(") # Getting this warning- Its correct
}
```
...
```{r}
# Switching back to plot mode- no longer interactive
tmap_mode("plot")
```
...
aggregating_zones_area_weighted <- st_interpolate_aw( x = incongruent_2,
                                                      aggregating_zones,
                                                      # Splitting area (count) by Proportions
                                                      extensive = TRUE) # used when working with density and rate
...
# Creates layout of the states in the west without the polygons in between
us_west_union <- st_union(us_west) 

# Combine geometeries of Texas and Western States
texas <- us_states[us_states$NAME == "Texas",]

# This combines both the western union and texas together
...
# st_make_valide - will print out output without error message `invalid`
...
```{r}
# nz = new zeland data - Finding the Middle point - *Multi-polygons*
nz_centroid <- st_centroid(nz)
# seine = seine data - Finding the Middle point - *Only a line*
seine_centroid <- st_centroid(seine)

# Ensure that the centroid is placed onto the object
...
map1 <- tm_shape(nz) +
  ...
  tm_shape(nz_pos) +
  # Corrects the dot to be on the lines
...
```{r}
#Any point that is farther I want to keep and I want to adjust by that
