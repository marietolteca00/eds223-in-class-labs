...
world_elevation <- read_stars(here("wk1-git","week1","data","worldelevation.tif"))

# country boundaries - vector data
world_vector <- read_sf(here("wk1-git","week1","data","worldvector.gpkg"))

# major cities - vector data
world_cities <- read_sf(here("wk1-git","week1","data","worldcities.gpkg"))
...
tm_shape(world_vector) +
  tm_dots() + # defines the spatial object to plot
  tm_polygons()
...
tm_shape(world_elevation) + # Plotting global  world elevation data

  # Because this is a raster use
  tm_raster()+ # tm_raster for raster data
  
  # Add vector to the shape
  tm_shape(world_vector) +
  # outline of our vector
  tm_borders() + # adds a border to the world vector
  # Adds in cities
  tm_shape(world_cities) +
  
  tm_dots()+
  # the data has a cities name in a column
  tm_text("name", size = 0.7, col = "blue") # changing color of the names and size
```
...
```{r}
#Layout matters - It will keep stacking on-top
tm_shape(world_cities)+
  tm_dots() + 
  tm_text("name")+
...
tm_shape(world_elevation)+
  ...
  tm_shape(world_vector)+
  tm_borders()+
  # Add `is.main= TRUE` so the computer knows to keep it layered on top
  tm_shape(world_cities, is.main = TRUE) + # Plotting world_cities last, but set as a main shape
  tm_dots()+
  tm_text("name")
```
...
```{r}
# can use an external tool to located the type of coordinates to locate
...
```{r}
# Only fills in polygons without border outline
# keeps shape of map
...
```{r}
tm_shape(world_cities) +
  # Only plots circles in world cities
  tm_symbols()
```
...
- **Look at syntax for `tm_symbols()` **

```{r}
tm_shape(world_cities) +
  # tm_squares()
  # tm_symbols(shape = 22) # another way to add squares
  # tm_bubbles()
  tm_dots()
```
...
tm_shape(world_vector)+
  tm_polygons(fill = "life_expectancy",
              ...
              fill.scale = tm_scale(values = "brewer.Y1OrBr"),
              fill.legend = tm_legend(title = "Life Expectancy (years)"))
```
...
tm_shape(world_vector)+
  tm_fill()+
  tm_scalebar()+
  # adding a north arrow - Automatically assumes North
  # Can adjust by using `north= degree`
...
tm_shape(world_vector)+
  ...
  tm_title(text = "Global Income", 
           # Add a box around title
           frame = TRUE) +
  tm_layout(color_vision_deficiency_sim = ("protan"),
    text.fontfamily = "American Typewriter",
            style = "beaver",
            bg.color = "lightblue",
...
```{r}
# Run this to make it interactive!
tmap_mode("view")
```
...
# To save on desktop
tmap_save(map1, here::here("week1","~/Desktop/tmap-example.png"))
